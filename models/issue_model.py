...
def get_all_issues(filters=None, page=1, limit=10, sort="created_at", order="desc"):
    ...
    # Pagination + sort
    cursor = mongo.db.issues.find(query).sort(sort, sort_order).skip((page - 1) * limit).limit(limit)
...
def delete_issue(issue_id):
    result = mongo.db.issues.delete_one({"_id": ObjectId(issue_id)})
    return result.deleted_count > 0


# Add comment
def add_comment(issue_id, comment_text, author):
    now = datetime.utcnow()
    ...
    mongo.db.issues.update_one(
        {"_id": issue_id},    # <- don't wrap with ObjectId
        {"$push": {"comments": comment}}
    )
    return comment_id

def get_comments(issue_id):
    issue = mongo.db.issues.find_one({"_id": issue_id})  # <- same
    if not issue or "comments" not in issue:
        return []
    return sorted(issue["comments"], key=lambda x: x["created_at"], reverse=True)
